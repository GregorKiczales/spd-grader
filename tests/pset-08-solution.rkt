...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname pset-08-solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
...
;; produce a list of the names of all of the brown cats
(check-expect (brown-cat-names empty) empty) 
(check-expect (brown-cat-names LOC1) 
...
;; produce the number of cats in loc with names less than n letters long
(check-expect (count-short-names empty 5) 0)
(check-expect (count-short-names LOC1 4) 2)
...
(@template fn-composition use-abstract-fn Cat)

(define (sum-age-c loc c)
  (local [(define (color-equal? cat)
            (string=? (cat-color cat) c))]
    (foldr + 0 (map cat-age (filter color-equal? loc)))))
...
;; for Treasure that can be found in a Scavenger Hunt.
;;
...
(check-expect (fold-treasure make-treasure
                             cons
                             "buried"
                             "sunken"
                             "locked"
...
;; produce the labels of treasure with greater than n gold after multiplier
...
(define (more-than-n t n)
  (local [(define (c1 label amount diff rfns rfnlot)
            (if (> (* amount rfns) n)
                (cons label rfnlot)
                rfnlot))]
