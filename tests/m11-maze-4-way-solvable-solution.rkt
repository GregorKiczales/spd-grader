...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname m11-maze-4-way-solvable-solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
(@assignment lectures/m11-maze-4-way-solvable)
(@problem 1)


;; PROBLEM - REVISE THIS PROGRAM SO THAT IT:
;;  - works for mazes like M7
;;  - produces a path of positions to solution (or fails)
...
(define (solve m)
  (local [(define R (sqrt (length m)))
          ...
          ;; trival: reaches lower right, previously seen position,
	  ;;         or no more moves
          ;; reduction: move down and right if possible
          ;; argument: maze is finite, so moving down and right
          ;;           will eventually reach solution or dead end
	  ...
	  ;;         or no more moves
          ...
          ;; todo is    (listof Pos); worklist
          ;; visited is (listof Pos); every position ever visited
          (define (solve/p p todo visited)
            (cond [(solved? p) true]
                  [(member p visited) (solve/lop todo visited)]
                  [else
                   (solve/lop (append (next-ps p) todo)
                              (cons p visited))]))

          (define (solve/lop todo visited)
            (cond [(empty? todo) false]
                  [else
                   (solve/p (first todo) (rest todo) visited)]))
    ...
    (solve/p (make-pos 0 0) empty empty)))
