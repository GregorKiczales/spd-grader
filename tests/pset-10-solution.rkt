...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname pset-10-solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
...
(check-expect (filter-credits-course-num C110 200 4) (list C319 C310 C221 C213 C210))

(@template Course ListOfCourse accumulator)
            

(define (filter-credits-course-num c0 course-num num-credits)
  ;; c-wl is (listof Course): worklist
...
(define (max-credit-path c0)
  ;; c-wl is (listof Course)
  ;; course worklist
  ;; p-wl is (listof (listof Course))
  ;; courses on path in the tree to corresponding element of c-wl
  ;; INVARIANT: c-wl and p-wl always have same length and corresponding elements
  ...
  (local [(define (fn-for-course c path c-wl p-wl rsf)
            (local [(define npath (cons c path))]
              (if (empty? (course-dependents c))
                  (fn-for-loc c-wl p-wl (max-path npath rsf))
                  (fn-for-loc
		   (append                         (course-dependents c)  c-wl)
		   (append (map (lambda (s) npath) (course-dependents c)) p-wl)
		   rsf))))
