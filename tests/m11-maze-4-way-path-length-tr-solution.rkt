...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname m11-maze-4-way-path-length-tr-solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)

(@assignment lectures/m11-maze-4-way-path-length-tr)
(@problem 1)
  ...
  (local [(define R (sqrt (length m)))
	  ...
	  ;;            maze to reach the nth element of p-wl
          ...
          (define (solve/p p c p-wl c-wl visited)
		    ...
		    (append (map (lambda (p1) (add1 c)) (next-ps p)) c-wl)
		    (cons p visited))]))
    ...
    (solve/p (make-pos 0 0) 0 empty empty empty)))
