...
(define (solve m)
  (local [(define R (sqrt (length m)))
	  ...
	  ;;            is the length of that path
          ...
          ;;                 initialized to 0, because no path can
          ;;                 be that short
          (define (solve/p p path c p-wl path-wl c-wl rsf)
            (cond [(solved? p)
                   (solve/lop p-wl path-wl c-wl (choose rsf (add1 c)))]
                  [(member p path)
          ...
          (define (solve/lop p-wl path-wl c-wl rsf)
            (cond [(empty? p-wl) (if (zero? rsf) false rsf)]
                  [else
                   (solve/p (first p-wl)
			    (first path-wl)
			    (first c-wl)
    ...
    (solve/p (make-pos 0 0) empty 0 empty empty empty 0)))
