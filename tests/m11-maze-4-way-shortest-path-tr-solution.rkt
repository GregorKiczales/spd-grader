...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname m11-maze-4-way-shortest-path-tr-solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
...
(check-expect (solve M6) (list (make-pos 0 0)
                               ...
                               (make-pos 9 8)
                               (make-pos 9 9)))
(check-expect (solve M7) (list (make-pos 0 0)
                               (make-pos 0 1)
                               (make-pos 1 1)
                               (make-pos 2 1)
                               (make-pos 3 1)
...
(define (solve m)
  (local [(define R (sqrt (length m)))
            ...
            (cond [(solved? p)
                  ...
                  [else
                   (solve/lop (append (next-ps p)
                                      p-wl)
                              (append (map (lambda (x) (cons p path)) (next-ps p))
                                      path-wl)
                              rsf)]))
          ...
          (define (choose rsf npath)
            (cond [(empty? rsf) npath]
                  [(<= (length rsf) (length npath)) rsf]
                  [else npath]))
    ...
    (solve/p (make-pos 0 0) empty empty empty empty)))
